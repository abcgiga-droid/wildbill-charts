...
- Sources: DOW30, NDQ100, SP500, ARKK, CRYPTO, ETF100, STOCKS, and XLI..XLE use static JSON files at `watchlist-static-files.web.app`; DOW30/NDQ100/SP500 fall back independently to their corresponding Wikipedia tables. Full membership is stored in `symbols`; only `priceSymbols` are staged from Yahoo. CRYPTO, ETF100 and STOCKS each have 100 price symbols (STOCKS membership is the 1,696 Yahoo-fetchable tickers of the 4,000-row stocks-by-market-cap file; foreign-venue suffixes like `.ST`/`.AX` are excluded because Yahoo uses different symbols there). R2000 remains a local sample because the site has no equivalent list.
